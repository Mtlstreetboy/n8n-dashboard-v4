#!/usr/bin/env python3
"""
Script pour tester différents paramètres de détection de shift
"""
import sys
...
def test_with_params(ticker, threshold=1.5, window=14, min_articles=2):
    """Teste avec paramètres ajustables"""
...
if __name__ == '__main__':
    ...
    print(f"Analyse {ticker} - Seuil: {threshold}σ, Fenêtre: {window}j, Min: {min_art} articles")
    print("=" * 80)
    ...
    if 'error' in result:
        ...
        if result['outliers']:
            ...
            for i, o in enumerate(result['outliers'][:5], 1):
                ...
                print(f"  {i}. [{o.outlier_weight:.2f}σ] {icon}{recent} {o.article['title'][:60]}")
        
        if result['shift']:
            s = result['shift']
            print(f"\n⚠️  SHIFT DÉTECTÉ: {s['interpretation']}")
            print(f"    Force: {s['strength']:.2f}σ | Confiance: {s['confidence']*100:.0f}%")
