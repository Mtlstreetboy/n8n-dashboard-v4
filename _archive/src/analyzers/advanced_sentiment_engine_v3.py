#!/usr/bin/env python3
"""
⚠️⚠️⚠️ MOTEUR DE SENTIMENT MULTI-DIMENSIONNEL V3 - AVEC ANALYST INSIGHTS
--------------------------------------------------------------------
Version V3 avec 6 DIMENSIONS:
- 📰 Sentiment News (LLM-enhanced)
- 📊 Sentiment Options (métriques avancées)
- 🎯 Sentiment Analysts (Yahoo Finance Insights) ⬅️ NOUVEAU
- 🌪️ Détection de régime de volatilité
- 🔍 Détection automatique de catalyseurs
- 🚨 Système d'alertes intelligentes
- 🤖 Configuration adaptative par ticker via LLM
- 📊 Métriques avancées avec interprétation LLM

Basé sur V2 avec ajout de la dimension Analyst Insights
"""
import sys
...
class AdvancedSentimentEngineV3:
        ...
        for article in articles:
            ...
            signal = SentimentSignal(
                ...
                metadata={
                    ...
                    'method': 'llm' if 'sentiment_score' in llm_data else 'keyword'
                }
            )
...
- NVDA: decay moyen (3j), threshold moyen (0.4), tech volatile"""
        ...
        try:
            response_text = _ollama_generate(model="llama3", prompt=prompt, temperature=0.3, timeout=30)
...
Format: <emoji> <insight clé> | <action recommandée>"""
        ...
        try:
            interpretation = _ollama_generate(model="llama3", prompt=prompt, temperature=0.5, timeout=30).strip()
    ...
    def generate_master_sentiment(self) -> Dict:
        """🧠 MOTEUR PRINCIPAL V3: Combine 6 DIMENSIONS pour un score de sentiment ultime"""
        print(f"\n{'='*60}")
        print(f"🔍 ANALYSE MULTI-DIMENSIONNELLE V3: {self.ticker}")
        print(f"{'='*60}\n")
    ...
    def _print_report(self, report: Dict):
        ...
        print(f"\n{'='*60}")
        print(f"📊 RAPPORT DE SENTIMENT V3: {self.ticker}")
        print(f"{'='*60}\n")
    ...
    def _save_report(self, report: Dict):
        ...
        # Find existing timestamped snapshots for this ticker (V3)
        import glob
        pattern = os.path.join(OUTPUT_DIR, f"{self.ticker}_sentiment_v3_*.json")
        ...
        if create_new_snapshot:
            filename = f"{self.ticker}_sentiment_v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            filepath = os.path.join(OUTPUT_DIR, filename)
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(report, f, indent=2, ensure_ascii=False)
            print(f"💾 Rapport V3 sauvegardé: {filepath}")
        else:
            print(f"ℹ️ Snapshot existant récent trouvé ({os.path.basename(latest_existing)}); pas de nouveau snapshot horodaté créé.")

        # Always update the latest_v3.json pointer
        latest_path = os.path.join(OUTPUT_DIR, f"{self.ticker}_latest_v3.json")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 advanced_sentiment_engine_v3.py TICKER [--no-llm]")
        sys.exit(1)
    
    ticker = sys.argv[1].upper()
    use_llm = '--no-llm' not in sys.argv
    
    engine = AdvancedSentimentEngineV3(ticker, use_llm_config=use_llm)
